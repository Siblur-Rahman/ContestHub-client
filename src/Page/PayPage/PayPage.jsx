...
const PayPage = () => {
    ...
    try {
      if (contest?.Quantity <= 0) {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/contest/update/${id}`,
          {
            method: "PUT",
            headers: {
        ...
        console.log(formData)
          axios
            .put(`${import.meta.env.VITE_API_URL}user/update/${user?.uid}`, formData)
            .then((res) => {
  ...
  useEffect(() => {
    if (id) {
      fetch(`${import.meta.env.VITE_API_URL}/contest/${id}`)
